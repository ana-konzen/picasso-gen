...
function draw() {
  ...
  image(hairs[hairParams.type], width/2, height/2, hairs[hairParams.type].width / 2.8,  hairs[hairParams.type].height / 2.8);
...
function drawFeature(arr, params){
  ...
  const posx = map(params.position.x, -100, 100, width/2 - 150, width/2 + 150);
  const posy = map(params.position.y, -100, 100, height/2 - 250,  height/2 + 150);
  push();
  translate(posx, posy);
  if(params.flip){
    scale(-1, 1);
